...
for (const width of [390, 1440]) {
  test(`populated demo, cited assistant and decisions at ${width}px`, async ({
    page,
  }) => {
    const errors = [];
    ...
    await page.goto("/");
    await expect(
      page.getByRole("button", { name: "Open preloaded demo" }),
    ).toBeVisible();
    await page.getByRole("button", { name: "Papers", exact: true }).click();
    await expect(
      page.getByRole("heading", { name: "Check it. Then trust it." }),
    ).toBeVisible();
    await page.getByRole("button", { name: "Records", exact: true }).click();
    await page.evaluate(() => window.scrollTo(0, 0));
    await page.screenshot({
      path: `test-results/demo/landing-${width}.png`,
      fullPage: true,
    });
    expect(
      await page.evaluate(
    ...
    await expect(
      page.getByRole("region", { name: "Chat with Orbit" }),
    ).toBeVisible();
    ...
    for (const chat of await (
      await page.request.get("/api/personal/chats")
    ).json()) {
      await page.request.delete(`/api/personal/chats/${chat.id}`);
    }
    await page.reload();
    await navigate("Dashboard");
    await page
      .getByRole("button", { name: "Manage subjects & marks", exact: true })
      .click();
    await expect(page.getByRole("heading", { name: /CSE/ })).toBeVisible();
    await expect(
      page.getByText("Reference result", { exact: false }).first(),
    ).toBeVisible();
    await page.getByLabel("View semester", { exact: true }).selectOption("4");
    await expect(page.locator(".subject-list > li")).toHaveCount(10);
    await page
      .getByRole("combobox", { name: "View semester" })
      .selectOption("1");
    await expect(page.locator(".subject-list")).toContainText(
      "APPLIED CHEMISTRY",
    );
    await page
      .getByRole("combobox", { name: "View semester" })
    ...
    await page.evaluate(() => window.scrollTo(0, 0));
    await page.screenshot({
      path: `test-results/demo/academics-${width}.png`,
      fullPage: true,
    });
    await navigate("Chat");
    ...
    await page
      ...
      .fill("SQL test Friday, two hours");
    await page.getByRole("button", { name: "Ask Orbit", exact: true }).click();
    await expect(page.locator(".chat-thread article.assistant")).toContainText(
      "120-minute",
    );
    await expect(page.locator(".chat-thread article.assistant")).toContainText(
      "8/10",
    ...
    await page.evaluate(() => window.scrollTo(0, 0));
    await page.screenshot({
      path: `test-results/demo/assistant-${width}.png`,
      fullPage: true,
    });
    await navigate("Dashboard");
    await page
      .getByRole("button", { name: "Suggested actions", exact: true })
      .click();
    ...
    if (width === 390) {
      ...
      await suggestions
        ...
        .click();
      await expect(
        suggestions.getByRole("button", { name: "Accept action" }),
      ).toHaveCount(0);
      ...
      await page.request.put(`/api/assessments/${mark.id}`, {
        data: { ...body, score: 7 },
      });
      await page.request.put(`/api/assessments/${mark.id}`, {
        data: {
          ...body,
          score: 8,
          title: "Illustrative demo · SQL quiz (reviewed)",
        },
      });
    } else {
      await suggestions
        ...
        .click();
      await suggestions
        .getByRole("button", { name: "Accept action" })
        .last()
        .click();
